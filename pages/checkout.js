...
let b;
    var options = {
      ...
      handler: async function (response) {
        ...
        let x={ORDERID:oid,STATUS:t.success}
       
       let c=await fetch("/api/postTransaction", { 
          method: 'POST',
          headers: {
            'Content-type': 'application/json'
      ...
      b=await c.json();
       let y=JSON.parse(b.orderId)
       
       router.push(`/order?id=${y}&clearCart=1`)
      
        // router.push(`/order?id=${order._id}&clearCart=1`)
        // alert(response.razorpay_order_id);
        // alert(response.razorpay_signature);
      },
      prefill: {
        name: "OnlineDukaan Pvt Ltd",
        email: "achintyamishra01@gmail.com",
        contact: "9415527815",
      },
    };
    ...
    <>
      ...
      <div className='container m-auto w-4/5 '>
        ...
        <div className="   bg-gradient-to-b from-slate-100 to-pink-200 ... p-6 m-2  " >
          ...
          <div className="mx-8">
            <Link href={"/checkout"}><button disabled={disabled} onClick={makePayment} className="disabled:bg-indigo-300  flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8  focus:outline-none hover:bg-indigo-600 rounded text-sm mr-1">Pay ₹{subTotal} </button>
            </Link>
          </div>
